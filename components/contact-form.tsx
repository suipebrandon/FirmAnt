...
export function ContactForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 rounded-lg border border-ink/10 bg-white p-5 shadow-soft">
      ...
      <Field label="Project details" error={errors.details?.message}>
        <textarea className="field min-h-32" {...register("details")} placeholder="Tell us what you want to build, renovate or design." />
      </Field>
      {sent && (
      ...
      <button className="focus-ring inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-ink px-5 py-3 text-sm font-semibold text-white transition hover:bg-graphite" type="submit">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <Button variant="secondary" href={`mailto:${contact.email}?subject=Firm Ant quote request&body=${encodeURIComponent(message)}`}>
      ...
      <style jsx>{`
        .field {
          ...
          border: 1px solid rgb(23 32 27 / 0.15);
          background: white;
          padding: 0.75rem;
          outline: none;
        }
        .field:focus {
          border-color: #99000a;
          box-shadow: 0 0 0 3px rgb(201 21 33 / 0.16);
        }
      `}</style>
    </form>
  );
